...
public class QSSettingsBaseViewController: UITableViewController {
    ...
    @objc fileprivate func singleTapHandler(gesture: UITapGestureRecognizer) {
...
extension QSSettingsViewController: QSSettingsOptionsViewControllerDelegate {
    ...
    func settingsOptionsViewController(settingsVC settingsVc: QSSettingsOptionsViewController, didSelect option: String, for key: String) {
...
extension QSSettingsViewController {
    ...
    @objc fileprivate func switchValueChanged(theSwitch: UISwitch) {
        if let key = theSwitch.restorationIdentifier, let _ = root.setting(for: key) {
            defaultsStore.set(theSwitch.isOn, forKey: key)
            delegate?.settingsViewController(settingsVc: self, didUpdateSetting: key)
        }
    }
    ...
    private func configureToggleCell(cell: QSSettingsTableCell, setting: QSToggle) {
        ...
        s.addTarget(self, action: #selector(switchValueChanged(theSwitch:)), for: .valueChanged)
    ...
    private func settingCell(from tableView: UITableView, for setting: QSSettable,
                             atTopLevel: Bool, forRowAt indexPath: IndexPath) -> UITableViewCell {
        switch setting {
